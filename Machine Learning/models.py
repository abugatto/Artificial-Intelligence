...
class RegressionModel(Model):
    ...
    def __init__(self):
        ...
        #hyperparameters
        self.learning_rate = .2 #diverges at: (d=1: <=.13, d=2: <=.14, d=3: <=.43)
        self.depth = 3
    ...
    def run(self, x, y=None):
        ...
        def createNN():
            ...
            self.graph = nn.Graph(self.weights + self.bias) #weight + bias is variable vector
        ...
        if y is not None:
            ...
            return self.graph.get_output(self.graph.get_nodes()[-2]) #returns the prediction and the loss
...
class OddRegressionModel(Model):
    ...
    def run(self, x, y=None):
        ...
        if y is not None:
            ...
            odd_xm = nn.MatrixMultiply(self.graph, self.weights[0], odd_input_x)
            odd_xm_plus_b = nn.MatrixVectorAdd(self.graph, odd_xm, self.bias[0])

            #create the remaining layers:
            for i in range(1, self.depth):
                #add nonlinearity for previous layer:
                relu = nn.ReLU(self.graph, xm_plus_b)
                odd_relu = nn.MatrixMultiply(self.graph, nn.ReLU(self.graph, odd_xm_plus_b), -1)
